...
async function getDatesOfSale() {
    ...
    const itemsOfPage = await page.evaluate(() => Array.from(document.querySelectorAll('span'), element => element.textContent));

    console.log(itemsOfPage);

    const regex = /Recebimento de propostas:  Até \d{2}\/\d{2}\/\d{4} \d{2}:\d{2}:\d{2}\./;
...
module.exports = { getImoveis, getDatesOfSale };